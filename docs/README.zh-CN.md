<div align="center">
  <img align="center" width="180" src="../images/icon.png" alt="Pinia logo">
</div>
<br/>
<p align="center" style="font-size: 32px">日志系统</p>
<br/>

中文 | [English](../README.md)

> **本仓库 fork 自 [wutiange/log-record](https://github.com/wutiange/log-record)。**
> 上游原有行为保持不变，在此基础上补了一批日常调试用的功能，
> 完整清单见 [第 4 节](#4-本-fork-的增强)。

## 1. 介绍

用于在 Release 环境下查看日志的系统。这个系统的诞生源于实际开发过程中遇到的几个具体问题：

1. 测试工程师在测试过程中发现问题时，往往难以准确判断问题的来源；
2. 由于责任归属不明确，许多问题都被默认记录给了前端开发人员；
3. 一些偶发性问题出现一次后可能短期内无法复现，这给问题定位带来了很大困扰；
4. 对于网络日志每次使用 charles 过于繁琐。

通过这个日志系统，我们可以直接查看日志来定位问题，无需测试人员反复尝试复现。为了让系统发挥最大效用，我们建议在日常开发中尽可能详细地记录日志信息，这样可以更好地帮助我们排查和解决各类问题。

<span style="color: red; font-style:italic">*特别提醒：尽量不要在线上使用，除非你进行了特别完善的测试。</span>

## 2. 集成

在使用日志系统之前，你首先需要集成 [log-listener-plugin](https://github.com/wutiange/log-listener-plugin) 到你的项目。具体集成教程请参阅对应的文档。

[log-listener-plugin](https://github.com/wutiange/log-listener-plugin) 只支持 [react-native](https://www.reactnative.dev/) 项目。

## 3. 使用

我以 `macos` 为例。首先进入这个网站下载对应版本的软件，尽量下载最新的版本，因为新版修复了之前的很多 bug ：

<https://github.com/Kieatd/log-record/releases>

![](../images/docs/1729170936543.jpg)

进入后下载最新版本即可。下载后解压：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/9a07712aa77a4b8a83af341dc90465db~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image#?w=1840&h=872&s=129858&e=png&b=e3e6e7)

双击打开日志系统。但是一般会提示：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/001a6cd2b44b45999e664cf0c718c50a~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image#?w=520&h=524&s=78682&e=png&b=9b9ea0)

这个时候不要慌，打开“设置”，然后再打开“隐私与安全性”：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/7fd784e63aae4511b1e1de513d6f698b~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image#?w=1430&h=1402&s=385111&e=png&b=ede5e3)

然后在出现的弹框中点击打开就完成了。接下来只要日志上报这上面就会显示了。要想正常显示在这上面，还需要注意几点，如果这个系统你是在你的电脑上打开的，那么你得保证你的手机 WiFi 跟你的电脑处于同一网络；在 `App` 中成功设置了日志上报的 `testUrl` 。打开之后长这样：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3009e5c54cd04fffaf8b30fd340d00f1~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image#?w=2400&h=1200&s=84904&e=png&b=ffffff)

如果此时有日志，那么就会变成这样：

![](../images/docs/1729169366443.jpg)

其中每一条日志旁边都有一个横条，这个横条代表日志等级，也就是 `log ,warn, error` 这三种日志类型，见图：

![](../images/docs/1729170457548.jpg)


而第二部分则是时间，可以根据这个看出打印的时间。

右下角有两个按钮，第一个是清空当前屏日志，第二个按钮是保持滑动，也就是会跟随日志的增加而滚动。

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/45e4c77efa7048d38397de6441eca5c0~tplv-k3u1fbpfcp-jj-mark:0:0:0:0:q75.image#?w=1190&h=580&s=67234&e=png&b=fefefe)

其中还有一个很好的功能就是搜索，搜索支持同时搜，也就是既有又有的逻辑。打个比方我想搜索包含“App”的日志：

![](../images/docs/1729170589153.jpg)

我发现日志还是很多，于是我想基于“App”的日志基础上新增“background”这个日志，只需要使用空格分开两段文本就可以完成同事搜索：

![](../images/docs/1729170642853.jpg)

这个是我在平时的开发中有时候有这样的需求，所以写了这个。搜索除了这个功能外，还能过滤错误等级，手机型号等等功能，只要你 setBaseUrl 传入的类型有哪些那就你过滤就可以根据那些来进行，下面我演示根据日志等级过滤。

![](../images/docs/1729170800182.jpg)

其中 `level` 代表上报的字段名，`warn` 代表字段的对应值。这个格式是固定的，中间不能出现空格，空格都当做是且的关系。

目前支持了发现功能，具体可参考 [使用发现功能上报日志v1.2.x](1.2.x.zh-CN.md)。

## 4. 本 fork 的增强

上游功能保持原样，以下是实际每天用它调式时补上的功能。

### 4.1 网络接口列表

| 功能 | 说明 |
| --- | --- |
| **标记接口** | 在接口上右键选「标记」。标记过的接口在清除列表时**不会被清掉**，方便在噪声里盯住关心的几条；快速双击清除按钮则连标记一起清空。 |
| **耗时** | 每条显示「请求发出 → 收到响应」花了多久，例如 `123ms` / `2.50s`；还在等响应的留空。 |
| **列布局** | 序号 / 状态码 / 耗时 / 请求方法 装在一个框里，格子之间用竖线分隔；接口地址在框外，占满剩余宽度。 |
| **关键词高亮** | 按关键词规则给整行上色，每条规则带启用勾选框，可以临时关掉而不删配置。 |
| **忽略规则** | 把吵闹的接口整条隐藏（底部会提示隐藏了多少条）。 |
| **路径显示** | 隐藏开头的路径段让长 URL 更好读。隐藏的路径段不参与任何匹配，配置以「相对域名」形式保存，换域名仍然生效。 |
| **树 / 平铺视图** | 一键在「按路径分组的树」和「按到达顺序平铺的列表」之间切换。 |

### 4.2 接口详情

| 功能 | 说明 |
| --- | --- |
| **请求体 / 响应体查看器** | 支持搜索高亮、只看匹配、展开/折叠全部、整段复制、双击节点复制该节点的值。请求体、响应体、重新请求结果三处共用同一个组件。 |
| **重新请求** | 可以改请求头（只改值，字段名保持原样）和请求体，然后从电脑端重新发出去并就地看结果。请求由 Electron 主进程发出，不受 CORS 限制；结果块的布局和响应块一致。 |
| **一键复制** | 请求参数、请求体、响应体都能一键复制。 |

### 4.3 连接

| 功能 | 说明 |
| --- | --- |
| **标题栏显示本机 IP** | 显示在标题旁边，点击即复制。 |
| **全部 IP** | 「设置 → 连接说明」里会列出**所有**候选地址（推荐的在第一个），第一个连不上可以换一个试；同时列出**被排除**的地址（虚拟网卡 / VPN）。这样就不会显示 `10.8.0.x` 这种手机根本连不上的 VPN 地址。 |
| **排查提示** | 代理 / VPN 软件、Windows 防火墙弹窗的提醒。 |

### 4.4 窗口

窗口大小、位置和分栏宽度都会记住，下次打开保持一致。

### 4.5 设备（adb）

内置了一个 adb 面板，插上数据线就能在应用里直接操作手机，不用再开命令行。

| 功能 | 说明 |
| --- | --- |
| **adb 自动探测** | adb 不是系统自带程序，位置因人而异。会按「手动指定 → 应用内置 → ANDROID_HOME → 各系统 SDK 默认路径 → PATH」逐个找，找到后只用完整路径调用。全都没命中时界面上给「选择 adb 文件」按钮，选完记住；手动指定的路径坏了会直接报错，不会偷偷用别的 |
| **设备列表** | 型号 / 品牌 / Android 版本 / 连接方式 / 状态一目了然，多设备可切换。未授权时直接提示去手机上点「允许 USB 调试」 |
| **拖 APK 安装** | 把 .apk 拖到「安装应用」磁贴上就装好了。带进度百分比（先 push 到手机再 pm install）、已用时间、取消按钮，传输卡住 2 分钟会自动中止。可勾选「安装后自动打开」，装完直接把 App 拉到前台 |
| **卸载应用** | 列出已装应用（默认只列第三方，可勾选包含系统应用），支持搜索。卸载前二次确认，可选保留数据（-k） |
| **截图** | 截手机当前画面并预览，可保存到电脑。息屏时会明确提示（否则截出来是全黑的），并提供一键唤醒 |
| **屏幕常亮** | 插着电时不让屏幕熄灭（调试时不用一直戳屏幕）。开关打开时会顺便点亮屏幕，没插电时也会说明「暂时不生效」 |
| **无线调试** | 插线时点一次「开启」，之后拔线也能连；填手机 IP 即可连接 |
| **投屏操控** | 右侧面板显示手机实时画面，可以直接用鼠标点（轻触）、按住拖动（滑动）、滚轮（滚动），方向键/回车等键也能发过去。息屏时会说明并给「唤醒屏幕」按钮 |
| **Monkey 压测** | 磁贴左边点一下就开始往指定应用里随机点按滑动（跑稳定性），右边设置参数：目标应用（带应用名的下拉）、事件数量、间隔、随机种子（留着能复现问题）、忽略崩溃/无响应。输出里的 CRASH 会标红 |
| **传文件到手机** | 把文件拖到磁贴上就传（也可以点击选择，支持多选），带进度条和取消。默认传到 `Download` 目录，点磁贴上的文件夹图标可以改 |
| **填调试地址** | 电脑 IP 变了之后，原来得在手机上打开 App 的调试页、手动改地址、点设置、再重启 App。现在点一下磁贴左半边就全做了：用 adb 的 uiautomator 找到那个输入框 → 清空 → 填上本机 IP → 回读校验 → 点按钮 → 重启 App。不用改 App、不需要 root |
| **抓包代理** | 一键把手机的全局 `http_proxy` 指到本机（不需要 root），并可选地顺手重启目标 App —— 改完代理后已经跑着的 App 不一定立刻生效，这一步原来得手动做。设置里可以改端口、选要重启的 App；磁贴左半边再点一次就关掉代理。设置前会先探一下电脑上那个端口在不在监听，不在会提示 |
| **自定义命令** | 直接跑 adb shell 命令，输出进下方的面板 |

所有命令的原始输出都会汇总在页面下方的输出面板里，成功绿色、失败红色。

## 5. 计划

- [x] 优化日志太多的性能问题（1.0.6）；
- [x] 优化日志搜索（1.0.6）；
- [x] 优化网络日志的查看体验（1.0.7）；
- [x] 新增网络清空功能（1.0.7）；
- [x] 支持手动检查更新（1.0.7）；
- [x] 优化网络日志太多的性能问题，改善体验（1.0.8）；
- [x] 新增网络日志搜索（1.0.8）；
- [x] 新增连接日志系统提示（1.0.9）；
- [x] 新增英文版本（1.1.0）；
- [x] 新增黑暗模式（1.1.0）；
- [x] 新增发现模式（1.2.0）；
- [ ] 支持查看日志调用堆栈（1.3.0）；
- [ ] 支持显示崩溃日志（1.3.0）；
- [ ] 支持根据日志信息分析代码潜在的问题和优化建议（1.4.0）；